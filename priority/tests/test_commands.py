...
from priority.management.commands.calculate_priority import (
    Command, calculate_priority, get_priority_attribution_data,
    read_and_aggregate_sacct_dump, read_raw_sacct_dump,
...
class PriorityCalculationTests(PriorityCommandTests, TestCase):
    ...
    def test_calculate_priority(self):
        ...
        expected_columns = {
            'attribution_points', 'quality_of_service', 'account',
            'cpu_hours_to_date', 'gpu_hours_to_date', 'prioritised_cpu_hours',
            'prioritised_gpu_hours'
        }
        ...
        self.assertEqual(
            tuple(calculated_priority[test_columns].itertuples()),
            expected_priorities
        )
...
class PriorityAttributionDataTests(PriorityCommandTests, TestCase):
    ...
    def test_get_priority_attribution_data_with_existing_data(self):
        ...
        for original_record in original_data.itertuples():
            ...
            for field in (
                'cpu_hours_to_date', 'gpu_hours_to_date',
                'prioritised_cpu_hours', 'prioritised_gpu_hours',
                'quality_of_service', 'attribution_points'
            ):
                self.assertEqual(
    ...
    def test_get_priority_attribution_data_with_new_project(self):
        ...
        for key in (
            'cpu_hours_to_date', 'gpu_hours_to_date', 'prioritised_cpu_hours',
            'prioritised_gpu_hours', 'quality_of_service'
        ):
